...
export async function analyzeFoodEntry(input: string): Promise<FoodItem[]> {
  const completion = await groq.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: `You are a precise nutrition database. When given food descriptions, return a JSON object with an "items" array.
...
- Scale macros by quantity: "2 eggs" = 2× the values of 1 egg; "large" portions = larger values
